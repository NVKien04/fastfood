...
export const ProductCard = ({ product, onOpenDetail, onQuickAdd }: ProductCardProps) => {
  ...
  return (
    ...
    >
        ...
        {product.img ? (
          <div className="relative w-full h-full">
            <Image
              ...
              className="object-contain drop-shadow-[0_12px_24px_rgba(0,0,0,0.12)] dark:drop-shadow-[0_15px_30px_rgba(0,0,0,0.6)] group-hover:scale-105 transition-transform duration-300 ease-out"
            />
          </div>
        ) : (
      ...
      <div className="w-full min-h-11 flex items-center justify-center px-1 my-1.5">
      ...
      <div className="mt-auto w-full flex flex-col items-center justify-end min-h-14.5 gap-1.5 pb-1">
        {hasDiscount ? (
        ...
        >
          <span>{hasOptions ? `${t('PRODUCT.PRICE_FROM')} ${formatVND(currentPrice)}` : formatVND(currentPrice)}</span>
        </Button>
      </div>
    </div>
  );
