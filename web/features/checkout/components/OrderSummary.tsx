    ...
    <div className="bg-white rounded-3xl p-6 sm:p-8 border border-gray-100 shadow-xl shadow-gray-200/40 sticky top-24">
      ...
      {items.length === 0 ? (
        <div className="py-8 text-center text-gray-400">
          ...
          <Link
            href="/"
            className="inline-block mt-3 text-xs font-bold text-red-600 hover:underline"
          >
            ← Khám phá thực đơn
          </Link>
        </div>
        ...
        <div className="space-y-4 max-h-[340px] overflow-y-auto pr-1">
          {items.map((item) => (
            <div
              key={item.id}
              className="flex items-start justify-between gap-3 pb-3 border-b border-gray-50 last:border-b-0"
            >
              <div className="flex-1 min-w-0">
                ...
                {item.variant && (
                  <div className="text-[11px] text-gray-500 font-medium mt-0.5">
                    Phân loại: <span className="text-gray-700">{item.variant.name}</span>
                  </div>
                )}
              ...
              <div className="flex items-center gap-1.5 bg-gray-50 p-1 rounded-xl border border-gray-100">
                <button
                  ...
                  className="w-6 h-6 rounded-lg bg-white flex items-center justify-center text-gray-500 hover:text-red-600 shadow-xs transition-colors"
                >
      ...
      {/* Pricing Summary */}
      <div className="mt-6 pt-4 border-t border-gray-100 space-y-2.5 text-xs">
        <div className="flex justify-between text-gray-500 font-medium">
          <span>Tiền món ăn:</span>
          <span className="text-gray-900 font-bold">{formatVND(subTotal)}</span>
        </div>
        ...
        <div className="flex justify-between text-sm font-black text-gray-900 pt-3 border-t border-gray-100">
          ...
          <span className="text-lg font-black text-red-600">
            {items.length > 0 ? formatVND(total) : '0đ'}
          </span>
        </div>
      </div>
    </div>
