...
- Border radius is hierarchical: `{rounded.md}` (8px) for buttons, `{rounded.sm}` (6px) for inputs, `{rounded.lg}` (12px) for content + product cards, `{rounded.xl}` (16px) for the hero illustration container, `{rounded.pill}` for badges.
...
| `{rounded.sm}`   | 6px          | Small inline buttons, dropdown items, text inputs              |
| `{rounded.md}`   | 8px          | Standard CTA buttons, category tabs                            |
...
**`button-primary`** — The signature coral CTA. Background `{colors.primary}` (rgb(200, 16, 46)), text `{colors.on-primary}` (white), type `{typography.button}` (StyreneB 14px / 500), padding 12px × 24px, height 48px, rounded `{rounded.md}` (8px). Active state `button-primary-active` darkens to `{colors.primary-active}` (rgb(170, 10, 35)).

**`button-secondary`** — Background transparent, text `{colors.ink}`, 1px hairline border, same padding + height + radius as primary.
...
**`text-input`** — Standard text input. Background transparent, text `{colors.ink}`, type `{typography.body-md}`, rounded `{rounded.sm}` (6px), padding 12px × 16px (left padding 20px), height 48px. 1px border in `{colors.hairline}`. Hover state has border `{colors.primary}`. Focus state has border `{colors.secondary}` and no ring.

**`text-input-focused`** — Focus state. Border color shifts to `{colors.secondary}`. No focus ring.
