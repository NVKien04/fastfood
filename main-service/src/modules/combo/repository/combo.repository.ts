...
export class ComboRepository implements IComboRepository {
  ...
  async findById(id: string): Promise<CombosEntity | null> {
    try {
      return await this.repo.findOne({ where: { id } as FindOptionsWhere<CombosEntity>, relations: ['comboItems'] });
    } catch (error: any) {
  ...
  async findBySlug(slug: string): Promise<CombosEntity | null> {
    try {
      return await this.repo.findOne({ where: { slug } as FindOptionsWhere<CombosEntity>, relations: ['comboItems'] });
    } catch (error: any) {
      console.error(`[DB ERROR] findBySlug failed:`, error.message);
      throw new BadRequestException({
        errorCode: error.code,
        message: `Failed to fetch record with slug ${slug}`,
        detail: error.message,
      });
    }
  }
  ...
  async createMany(entity: DeepPartial<CombosEntity[]>, manager?: EntityManager): Promise<CombosEntity[]> {
    try {
      const repo = this.getRepo(manager);
      const entities = repo.create(entity as DeepPartial<CombosEntity>[]);
  ...
  async GetPage(filterObj?: any): Promise<PaginationResponse<any>> {
    console.log('🚀 ~ ComboRepository ~ GetPage ~ :');
    try {
