...
export class ProductIngredientRepository implements IProductIngredientRepository {
  ...
  async findById(id: string): Promise<ProductIngredientsEntity | null> {
    try {
      return await this.repo.findOne({ where: { id } as FindOptionsWhere<ProductIngredientsEntity> });
    } catch (error: any) {
  ...
  async createMany(
    ...
    try {
      const repo = this.getRepo(manager);
      const entities = repo.create(entity as DeepPartial<ProductIngredientsEntity>[]);
  ...
  async GetPage(filterObj?: any): Promise<PaginationResponse<any>> {
    console.log('🚀 ~ ProductIngredientRepository ~ GetPage ~ :');
    try {
